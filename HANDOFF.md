...
- ✅ **0.5.1 — zone-type differentiation** (requested by Andrea): irrigation types extended to spray/rotor/mp_rotator/microspray/drip/subsurface_drip; per-zone `cover` (none/organic_mulch/plastic_mulch) multiplies Kc (×1/.85/.75) AND effective rain (×1/1/0.3 — film blocks rainfall); drip calculator (line m × emitter L/h ÷ spacing cm ÷ area m²→mm/h) in the zone editor; `db.apply_migrations()` adds new SQLite columns on upgrade. SPEC §5.2 updated. **M6 generator must treat turf (spray/rotor) vs drip beds differently (windows, MAD, frequency — drip under film least frequent).**
- ✅ **Milestone 6** (rule-based generator) — `core/climate.py` (Open-Meteo Archive, 5y daily ET₀ → smoothed day-of-year curve → season boundaries at 2/4 mm/day crossings), `core/generator_rules.py` (proposal builder: Lawn Start fixed/manual + Spring/Summer/Autumn budget programs, pre-dawn summer window, MAD 40/50; mild/cold-climate fallbacks), `POST /api/programs/generate` + `/apply` (apply replaces only `generated_by=rules`), Programs tab UI (generate→preview+explanation→apply, simple editor; editing flips badge to manual per SPEC). Verified offline (synthetic climate) + live (Correzzana: spring 03-14, summer 05-23, autumn 08-22, end 10-06, peak 5.25 mm 06-22) + end-to-end in browser preview (apply → Summer active → recalc planned 2 pre-dawn runs with correct durations). Version 0.6.0. **Confirmed on Andrea's HA.** M6 leftovers: Lawn Start auto-expiry, first-run wizard integration (Setup hint → Programs), timeline strip, every_n_days UI.
- ✅ **Milestone 7** (operational controls) — `POST /api/system/on|off` (OFF also stop_all), `POST /api/system/rain_delay?hours=` (0 cancels), `POST /api/schedule/{id}/override?minutes=` (capped at zone max_runtime). Executor `run_schedule` re-checks system/rain-delay at execution time and skips with reason. Dashboard: ON/OFF toggle, 24/48/72h rain-delay buttons + cancel link, ✎ per-run duration override. Verified: 8/8 API checks + UI flows in preview. Version 0.7.0. **Pending test on Andrea's HA.** (Intensity slider + per-run skip already shipped in M5.)
- ✅ **Milestone 8** (HA entity exposure) — `core/expose.py`: `gather_state` (per-zone deficit/next-run + active zone/program + switch/number), `MqttExposer` (paho-mqtt 2.x, discovery retained under one device, availability topic, command topics for system_enabled/intensity handled on the main loop incl. stop_all on OFF), `RestExposer` fallback (read-only sensors), `detect_mqtt` via Supervisor services API, `refresh_job` every 60 s honoring the opt-in `expose_entities` config toggle (Setup UI). `config.yaml` gains `services: [mqtt:want]`. Verified 15/15 offline (fake MQTT client: discovery payloads, retained states, no re-discovery, command roundtrip) + Setup toggle in preview. Version 0.8.0. **Pending test on Andrea's HA (needs Mosquitto broker add-on for the MQTT path; REST fallback otherwise).**
- ✅ **0.8.1** — weather entity wired in (was stored-only until now): `ha.get_weather_daily_rain` (weather.get_forecasts w/ response via Supervisor), `engine.merge_forecast_rain` (overrides forecast dates only; past stays Open-Meteo), applied in daily_calc + engine/recalc with logged fallback on errors; `GET /api/weather/entity_test` + Test button in Setup. README gains the my.home-assistant.io add-repository badge.
- ✅ **Milestone 9** (AI generator) — `core/generator_ai.py`: providers ha_conversation/anthropic/openai (env AI_PROVIDER/AI_API_KEY/AI_MODEL from add-on options; defaults claude-sonnet-5 / gpt-4o), prompt = system rules + JSON bundle (config, zones, balance sample, climate bounds, rule-based baseline, user notes), strict ProgramCreate validation with one retry, `review()` for plain-text suggestions. API: `GET /api/programs/ai_info`, `/generate` engine=ai (falls back to rules with message on ANY AI failure → `engine_used: rules_fallback`), `/review`. `/apply` now replaces generated_by in (rules, ai). UI: violet "Generate with AI" + notes textarea + "Ask AI to review" (hidden when provider none). Hard rule kept: proposal-only, AI never actuates. Verified 13/13 offline (gating, parse/validate/reject, retry, bundle) + UI hiding in preview. Version 0.9.0. **Pending live AI test on Andrea's HA (set ai_provider in add-on options).**
- ✅ **Milestone 10** (i18n + docs + 1.0) — `it.json` full translation + `setLang`/`LANGUAGES` in i18n.js + language picker in Setup (reload on change; config.language drives the catalog at app start). DOCS.md rewritten as the complete in-HA guide: model formula, full Setup/Zones field reference with practical examples (sprinkler PR from catalog + catch-cup test, dripline calculator walkthrough), programs/generator/AI setup (OpenAI/Anthropic key steps, ha_conversation), dashboard, MQTT exposure with automation example, options, FAQ/troubleshooting, privacy. README marked 1.0. Version **1.0.0**, tagged v1.0.0 + GitHub release.

**🏁 PROJECT COMPLETE — all 10 milestones shipped.** Post-v1 roadmap ideas remain in SPEC §13 (plus deferred niceties noted per milestone above: WebSocket live updates, zone drag-reorder, map-pin picker, Lawn Start auto-expiry, moisture/rain-sensor planning checks, soak interleaving, timeline strip).
